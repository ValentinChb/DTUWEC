module individual_pitch_controller
    ! Control Dll of type 2:
    ! Dll for applying individual pitch control actions, based on cyclic flap controller by tkba 
    ! with Colemann transformed blade root bending moments.
    !
    ! original version v.1.0, 26/04/2018, by TKBA
    ! modified version v.1.0, 22/01/2020, by famen
    !
    ! ******************************* example htc input *******************************
    !begin type2_dll; 
    !    name individual_pitch_controller; 
    !    filename  ./control/individual_pitch_controller.dll; 
    !    dll_subroutine_init init_ipc_controller; 
    !    dll_subroutine_update update_ipc_controller; 
    ...
    !        constant  5   10.0  	 ; Max amplitude of cyclic pitch action [deg]
    ...
    !
    ! *********************************************************************************
    use individual_pitch_controller_fcns
    use write_version_mod
    implicit none
contains
!**************************************************************************************************
subroutine init_ipc_controller(array1,array2) bind(c, name='init_ipc_controller')
    ! Initialize individual pitch controller
    implicit none
    !!DEC$ ATTRIBUTES DLLEXPORT, C, ALIAS:'init_ipc_controller'::init_ipc_controller
    !DEC$ IF .NOT. DEFINED(__LINUX__)
    !DEC$ ATTRIBUTES DLLEXPORT :: init_ipc_controller
    !DEC$ END IF
    real(mk) array1(19),array2(1)
    ...
    ! output to log file
    fmt = '(6a17)'
    open(ctrl_logfile_unit, file='dtuwec_ipc.log')
    write(ctrl_logfile_unit,'(A)') ' Using '//TRIM(ADJUSTL(TextVersion))
    write(ctrl_logfile_unit,fmt) 'time','IPC blade1','IPC blade2','IPC blade3',&
                                        'MBC moment cos','MBC moment sin'
    return
end subroutine init_ipc_controller

!**************************************************************************************************
subroutine update_ipc_controller(array1,array2) bind(c,name='update_ipc_controller')
!!DEC$ ATTRIBUTES DLLEXPORT, C, ALIAS:'update_flap_controller_cyclic'::update_flap_controller_cyclic
    !DEC$ IF .NOT. DEFINED(__LINUX__)
    !DEC$ ATTRIBUTES DLLEXPORT :: update_ipc_controller
    !DEC$ END IF
    ...
    real*8 uref_mbc(3),uref_rot(3)              ! Reference control signal in fixed MBC frame and in rotation frame
    real(mk) ipc_switch                         ! switcher between partial loads and full load region 
    character*256 fmt
    ! --------------------------------------------------------------- !
    ! Main Body
    ! --------------------------------------------------------------- !
    ...
    ipc_switch = array1(7)                  ! read control region signal
    ...
    IF(ABS(psi_quad).lt.1.d-6)  THEN ! Null quadratic term, linear scheduling
        psi_now = psi_ref + (meanpitref-theta_gsref)*psi_lin    ! in (deg)
    ...
    ELSE                             ! turn ctrl on 
        ...
        CASE(0)     ! Turbine in normal production, ctrl is on.
            ...
            kgain = 1.d0
            uref_mbc(1) = 0.d0      ! Collective part of the ipc pitch angle is zero
            ...
            ! Apply the ipc switch interpolate factor 
            uref_rot(1:3) = uref_rot*ipc_switch
    ...
    ! Output
    ! --------------------------------------------------------------- !
    array2(1:3) = uref_rot(1:3)             ! Output for reference cyclic pitch angle 1,2,3 (rad)?  
    ...
    fmt = '(6f17.5)'
    write(ctrl_logfile_unit,fmt) time,array2(1:3)*raddeg,array2(6:7)
    return
end subroutine update_ipc_controller
!**************************************************************************************************
end module individual_pitch_controller
